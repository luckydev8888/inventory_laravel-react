...
import { axios_get_header, axios_post_header, axios_put_header } from "../request/apiRequests";
...
function PurchaseOrder() {
    ...
    const get_purchase_orders = () => {
        ...
        axios_get_header('/purchase_order/get_purchase_orders', decrypted_access_token)
        .then(response => {
    ...
    const generate_po_number = () => {
        axios_get_header('/purchase_order/generate_po_number', decrypted_access_token)
    ...
    const get_supplier_products = (supplier_id) => {
        axios_get_header('/supplier/get_supplier_products/' + supplier_id, decrypted_access_token)
    ...
    const get_purchase_order = (param, status) => {
        ...
        get_suppliers();
        axios_get_header('/purchase_order/get_purchase_order/' + param, decrypted_access_token)
        .then(response => {
            const data = response.data.purchase_order_data;
    ...
    const handleSubmit = (e) => {
        ...
        if (formData.supplier_id === '') {
            ...
            setLoading(true);
            if (editIndex === 1) {
                axios_put_header('/purchase_order/update_purchase_order/' + formData.po_number, formData, decrypted_access_token)
                .then(() => {
                    setLoading(false);
                    setDialog(false);
                    formDataReset();
                ...
                axios_post_header('/purchase_order/add_purchase_order', formData, decrypted_access_token)
                .then(() => {
                    setLoading(false);
                    setDialog(false);
                    formDataReset();
